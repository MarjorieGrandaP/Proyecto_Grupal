...
# URL de conexión utilizada por servicios administrados como Render.
...
if not DATABASE_URL and not DB_PASSWORD:
    raise RuntimeError("No se encontró DB_PASSWORD. " "Configúrala en el archivo .env.")
...
def inicializar_base_datos():
    ...
    if not DATABASE_URL:
        ...
        try:
            ...
            print(
                f"[AVISO INICIALIZACION] Verificación de base de datos 'postgres': {e}"
            )
    ...
    try:
        ...
        ruta_esquema = os.path.join(
            os.path.dirname(os.path.dirname(__file__)), "sql", "esquema.sql"
        )
        if os.path.exists(ruta_esquema):
            ...
            conn.commit()
            print(
                "[BD] Esquema de base de datos cargado correctamente desde sql/esquema.sql."
            )
